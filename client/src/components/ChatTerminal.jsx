...
const buildScreenshotMessage = (screenshot, agentId) => {
  if (!screenshot || !screenshot.object_path) {
    return null;
  }

  const timestamp = ensureDate(screenshot.uploaded_at);

  return {
    id: `screenshot-${screenshot.object_path}`,
    sender: 'agent',
    agentId: agentId || 'agent',
    text: screenshot.metadata?.description || 'Screenshot captured',
    timestamp,
    taskId: screenshot.task_id ?? null,
...
const ChatTerminal = () => {
  ...
  const fetchScreenshots = useCallback(async () => {
    try {
      const response = await fetch(`${API_BASE}/agents/live?limit_per_agent=10`);
      const data = await response.json();

      if (!response.ok) {
        throw new Error(data.detail || 'Failed to load screenshots');
      }
      ...
      if (Array.isArray(data.agents)) {
        data.agents.forEach((agent) => {
          if (Array.isArray(agent.screenshots)) {
            agent.screenshots.forEach((screenshot) => {
              const msg = buildScreenshotMessage(screenshot, agent.agent_id);
              if (msg) {
      ...
      if (screenshotMessages.length) {
        upsertMessages(screenshotMessages);
      }
    } catch (error) {
      if (abortRef.current) {
        return;
  ...
  useEffect(() => {
    ...
    const responsesIntervalId = setInterval(fetchAgentResponses, REFRESH_INTERVALS.chat);
    const screenshotsIntervalId = setInterval(fetchScreenshots, REFRESH_INTERVALS.liveFeed);

    return () => {
      abortRef.current = true;
      clearInterval(responsesIntervalId);
      clearInterval(screenshotsIntervalId);
    };
  }, [fetchAgentResponses, fetchScreenshots]);
