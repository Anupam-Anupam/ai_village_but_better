...
API Endpoints:
- POST /execute: Process tasks with Llama AI
...
# Initialize OpenAI client (using Llama via OpenAI API)
...
# This connects to this agent's own MongoDB database (agent3db)
db = AgentDatabase()
...
async def startup_event():
    ...
    db.set_config("memory_window_size", 20, "Maximum number of memories to keep")
    ...
    # Log agent startup
    db.log("info", "Agent 3 database initialized!")
    print("🤖 Agent 3 database initialized!")
...
async def execute_task(req: Request):
    ...
    Receives a task, processes it with Llama AI, stores in this agent's database,
    ...
    3. Process with Llama AI using memory context
    ...
    try:
        ...
        system_prompt = "You are Agent 3, a specialized Llama AI assistant. Use your memory for context."
        if memory_context:
            system_prompt += f"\n\nYour recent memories:\n{memory_context}"
        
        # Process with Llama AI (using OpenAI API for now)
        completion = client.chat.completions.create(
            model="gpt-4-turbo-preview",  # Using GPT-4 for now, can be changed to Llama
            messages=[
        ...
        db.update_task(task_id, {
            "status": "completed",
            "output_data": {"response": response, "model": "llama-ai"}
        })
        ...
        memory_window_size = db.get_config("memory_window_size", 20)
